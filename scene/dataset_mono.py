...
class MonoDataset_train(Dataset):
    ...
    def __init__(self, dataset_parms,
        ...
        self.normal_fix = os.listdir(join(self.data_folder, 'normals'))[0].split('.')[-1]
    ...
    def getitem(self, index, ignore_list):
        ...
        normal_path = join(self.data_folder, 'normals' ,name_idx + '.' + self.normal_fix)
        ...
        # color_normal = normal * mask + (1 - mask) * 255
        # normal = Image.fromarray(np.array(color_normal, dtype=np.byte), "RGB")
        # normal = torch.from_numpy(np.array(normal)) / 255.0
        normal = Image.fromarray(np.array(normal, dtype=np.byte), "RGB")
        normal = torch.from_numpy(np.array(normal)) / 255.0
        normal = normal.clamp(0.0, 1.0)
        data_item['normal'] = normal
