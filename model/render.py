...
from model.mesh_util import get_visibility, get_visibility_color

import model.render_utils as util
import torch
...
class Render:
    ...
    def get_rgb_image(self, cam_ids=[0, 2], bg='white'):
...
if __name__ == '__main__':
    a = torch.randn(82736, 3)
    b = torch.randn(165468, 3)
    v,f,c = load_obj('/home/teawhite/桌面/ICON/results/icon-filter/obj/5a6a25963db2f667441d5076972c207c_recon.obj')
    device = torch.device(f"cuda:{0}")
    render = Render(size=512, device=device)
    render.load_meshes(a,b)
    #print(v.shape,f.verts_idx.shape) torch.Size([82736, 3]) torch.Size([165468, 3])
    image_F = render.get_rgb_image()
    image = image_F[1].squeeze(0)
    #image_F = (0.5 * (1.0 + image_F[0].permute(1, 2, 0).detach().cpu().numpy()) * 255.0)
    torchvision.utils.save_image(image, 'output_1.png')
