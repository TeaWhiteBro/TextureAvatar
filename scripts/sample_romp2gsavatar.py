...
data_folder = ''
subject = 'ourpose_ubc_A1zaFTtYy+S'

all_image_path = join(data_folder, subject, 'images')
all_mask_path = join(data_folder, subject, 'masks')


all_normal_path = join(data_folder, subject, 'normals')


if snap:
    train_split_name = sorted(os.listdir(all_image_path))[0:445:4]
    test_split_name = sorted(os.listdir(all_image_path))[446:647:4]
    scene_length = len(os.listdir(all_image_path))
    train_list = list(range(scene_length))[0:445:4]
    test_list = list(range(scene_length))[446:647:4]
...
#             test_split_name.append(idx_name)
    ...
    train_list = list(range(0, scene_length, 4))
    

    test_list = list(range(2, scene_length, 4))
    

    train_split_name = []
    test_split_name = []
    sorted_files = sorted(os.listdir(all_image_path))
    for idx, idx_name in enumerate(sorted_files):
        if idx in train_list:
            train_split_name.append(idx_name)
        if idx in test_list:
            test_split_name.append(idx_name)
...
out_normal_path =join(out_path, 'normals')
...
test_normal_path =join(test_path, 'normals')
...
assert len(os.listdir(all_image_path)) == len(os.listdir(all_mask_path))

train_sum_dict = {}
for image_name in train_split_name:
    shutil.copy(join(all_image_path, image_name), join(out_image_path, image_name))
    shutil.copy(join(all_mask_path, image_name), join(out_mask_path, image_name))
    shutil.copy(join(all_normal_path, image_name), join(out_normal_path, image_name))

test_sum_dict = {}
for image_name in test_split_name:
    shutil.copy(join(all_image_path, image_name), join(test_image_path, image_name))
    shutil.copy(join(all_mask_path, image_name), join(test_mask_path, image_name))
    shutil.copy(join(all_normal_path, image_name), join(test_normal_path, image_name))
